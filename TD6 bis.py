...
class App:
    ...
    def __init__(self,Data):
        ...
        self.interface = Canvas(self.root, width = self.width, height = 100, bg= 'black')
        self.quitter = Button(self.root, text = 'quitter', command = self.root.destroy, fg= 'white', bg= 'dark grey', relief= 'raised')
        self.nouveau = Button(self.root, text = 'nouveau', command = lambda: self.update_entrelacs(), fg = 'white', bg= 'dark grey', relief= 'raised')
        self.croisements = Label(self.root, text= 'Croisements', fg= 'white', bg= 'black', relief= 'raised', font= ('Arial', 15))
        self.liste = Label(self.root, text= (' '+str(self.data.croisements)+' '), fg= 'white', bg= 'black', relief= 'raised', font= ('Arial', 15))
        self.x = 0
        self.y0 = self.h
        self.y = 100
        self.colors = Button(self.root, text= 'Colors', command= lambda: self.update_colors(), fg= 'white', bg= 'dark grey', relief= 'raised')
        self.quitter.grid(row= 3, column= 1)
        self.nouveau.grid(row= 3, column= 3)
        self.colors.grid(row= 3, column= 2)
        self.window = Frame(self.root)
        self.canvas.grid(row= 1, column= 1, columnspan= 3)
        self.interface.grid(row= 2, column= 1, rowspan= 2, columnspan= 3)
    ...
    def read_word(self, mot):
        ...
        for lettre in mot:
            if lettre != 'H'and lettre != 'U' and lettre != 'D' :
                ...
                self.canvas.create_line(self.x, self.y, self.x + self.w, self.y, fill = self.listeCouleur[self.c % len(self.listeCouleur)], width=2)
                self.x,self.y = self.x + self.w,self.y
            elif lettre == 'U' :
                self.canvas.create_line(self.x, self.y, self.x + self.w, self.y - self.h, fill = self.listeCouleur[self.c % len(self.listeCouleur)], width=2)
    ...
    def update_entrelacs(self):
        self.canvas.delete("all")                
        self.data.new_entrelacs()  
        self.taille_fenetre()               
        self.data.create_word()  
        self.c = 0     
        self.y0 = self.h                          
        self.draw_entrelacs() 
        self.liste.config(text = (' '+str(self.data.croisements)+' '))
        
    def taille_fenetre(self):
        self.h = self.height / (self.data.nbFils + 1)
        self.w = self.width / (2 * len(self.data.croisements) + 1)
...
class Data:
    ...
    def new_entrelacs(self):
        self.nbFils = rd.randint(2,maxNbFils)
        self.croisements = [rd.randint(0, self.nbFils - 2) for i in range(rd.randint(1,maxNbCroisement))]
